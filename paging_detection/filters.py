...
if __name__ == "__main__":
    ...
    print(f"Removed {zero_entries} entries pointing to page 0.")

    total_pts = 0
    excluded_pts = 0
    for node in graph.nodes.values():
        if node[str(PageTypes.PT)]:
            total_pts += 1
            if node["has_oob_entries"]:
                node[str(PageTypes.PT)] = False
                excluded_pts += 1
    print(f"Filtered {excluded_pts} PTs based on out of bounds entries {excluded_pts/total_pts:%}.")

    print("Pruning graph")
    pruned = prune_designations(graph, pages)
    print(f"Pruned {pruned} designations.")
